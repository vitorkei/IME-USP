...
/* EMF */
void EMF();
void *exec_proc_EMF(void*);
/* OK */ void insertEMF(cell*, cell*);
/**********************************************************************/
/*                                                                    */
/*                          Variáveis globais                         */
/*                                                                    */
...
void *exec_proc_FCFS(void *args)
{
  ...
  cpu = sched_getcpu();
  if(DEBUG == 1)
    fprintf(stderr, "\nProcesso de nome %s está utilizando a CPU %d.\n", p->name, cpu);
...
/* Insere células na lista ligada em ordem crescente de t0 */
/* Idêntico ao insertFCFS. Copiado por questão de organização*/
void insertEMF(cell *new, cell *head)
{
  cell *p, *q;
