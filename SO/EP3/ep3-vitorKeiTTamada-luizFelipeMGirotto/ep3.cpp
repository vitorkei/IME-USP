...
void executa(int interval) // if(t % interval == 0) imprime infos
{
  ...
  for(t = 0, i = 0; t <= endSim; t++, RCounter = (RCounter + 1) % 10)
  {
    ...
    for (j = 0; j < i; j++)
    {
      if (procVec[j].ti.size() > 0 && procVec[j].ti[procVec[j].t] == t)
      {
...
void optimal (int index, int newPage)
{
  ...
  for (i = 0; i < procVec.size(); i++)
    for (j = 0; j < procVec[i].acc_freq.size(); j++)
    {
      ...
      if (procVec[i].acc_freq[j] >= max_acc && procVec[i].page[j] >= 0 && pageTable.pa[procVec[i].page[j]])
      {
...
void clk(int index, int newPage)
{
  ...
  // Remove a página do processo do bitmap da memória física e do arquivo ep3.mem
  address = pageTable.address[clkv[iClk]];
  for (int k = 0; k < ceil((double) p / ((double) s)); k++)
  {
    phyVec[address + k] = false;
    writeMem(0, (address + k) * 4, -1);
  }
  pageTable.address[clkv[iClk]] = -1;
  pageTable.pa[clkv[iClk]] = false;
...
void LRU4 (int index, int newPage)
{
  ...
  for (i = 0; i < procVec.size(); i++)
    for (j = 0; j < procVec[i].age.size(); j++)
    {
      ...
      {
        if (binToDec(procVec[i].age[j]) <= min_count && procVec[i].page[j] >= 0 && pageTable.pa[procVec[i].page[j]])
        {
...
void freeProc(int index)
{
  ...
  for(j = 0; j < procVec[index].page.size(); j++)
  {
    ...
    if (pag == 2)
    {
      int queue_size = scq.size();
      int mem;
      for (int i = 0; i < queue_size; i++)
      {
        mem = scq.front();
        if (mem == procVec[index].page[j])
        {
          scq.pop();
          break;
        }
    ...
    if (pag == 3)
    {
      int vec_size = clkv.size();
      for (int i = 0; i < vec_size; i++)
      {
        if (clkv[i] == procVec[index].page[j])
        {
          clkv.erase(clkv.begin() + i);
          if (clkv.size() == 0)
            iClk = 0;
          else
            iClk = iClk % clkv.size();
