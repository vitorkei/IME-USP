...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        = busters.getObservationDistribution(noisyDistance)[trueDistance] =
        = P( noisyDistance | trueDistance ) = P( e_(t+1) | X_(t+1) )
        """
        emissionModel = busters.getObservationDistribution(noisyDistance) # P(e_(t+1)|X_(t+1))
        ...
        for p in self.legalPositions:
            ...
            # P( X_(t+1) | e_(1:(t+1)) ) = P( e_(t+1) | X_(t+1) ) * P( X_(t+1) | e_(1:t) )
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        for p in self.legalPositions:
            # Recebe a probabilidade de cada ação do fantasma: ir para cima, para baixo, para a direita, para a esquerda ou ficar parado
            # Se a probabilidade de alguma dessas ações for zero, ela não é armazenada
            aux = self.getPositionDistribution(self.setGhostPosition(gameState, p))
            
            # A probabilidade de o fantasma estar em uma célula p no instante t
            #  é a somatória das probabilidades de ele estar em uma célula vizinha de p no instante t-1
            #  e mover-se para p no instante t ou de já estar em p no instante t-1 e a ação ser não se mover no instnate t
            for newPos, prob in aux.items():
                newPosDist[newPos] += self.beliefs[p] * prob
